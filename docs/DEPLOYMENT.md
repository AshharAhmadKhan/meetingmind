...
### Deployment
- `scripts/deploy/deploy-all.{sh|ps1}` - Deploy everything
- `scripts/deploy/deploy-backend.{sh|ps1}` - Deploy backend only
- `scripts/deploy/deploy-frontend.{sh|ps1}` - Deploy frontend only

### Testing & Debugging
- `scripts/utils/trigger-processing.py` - Manually trigger meeting processing
- `scripts/utils/check-meeting-status.py` - Check meeting processing status
- `scripts/monitoring/check-aws-credits.py` - Check AWS credit balance
- `tests/integration/test-rate-limiting.py` - Test API rate limits

### Data Management
- `scripts/data/fix-duplicate-action-ids.py` - Fix duplicate action IDs (one-time)
- `scripts/data/regenerate-autopsies-rulebased.py` - Regenerate meeting autopsies
