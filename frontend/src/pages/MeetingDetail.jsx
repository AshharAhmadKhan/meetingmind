...
export default function MeetingDetail() {
  ...
  async function fetchMeeting() {
    try { setMeeting(await getMeeting(meetingId)) }
  ...
  if (loading) return (
    <div style={{minHeight:'100vh',background:'#0c0c09',display:'flex',alignItems:'center',justifyContent:'center'}}>
      <style>{css}</style>
      <div style={{width:22,height:22,border:'2px solid #2a2a20',borderTopColor:'#c8f04a',borderRadius:'50%',animation:'spin 1s linear infinite'}}/>
    </div>
  )

  if (error || !meeting) return (
    <div style={{minHeight:'100vh',background:'#0c0c09',display:'flex',alignItems:'center',justifyContent:'center',fontFamily:"'DM Mono',monospace"}}>
      <style>{css}</style>
    ...
    <div style={s.root}>
      ...
      <header style={s.hdr}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:16}}>
          <span style={{fontSize:10,letterSpacing:'0.1em',color:'#c8f04a'}}>● Done</span>
          {dateStr && <span style={{fontSize:10,color:'#444438',letterSpacing:'0.06em'}}>{dateStr}</span>}
        </div>
      </header>
        ...
        {actions.length > 0 && (
          <div style={s.prog}>
            <div style={{display:'flex',justifyContent:'space-between',marginBottom:7}}>
              <span style={{fontSize:9,letterSpacing:'0.14em',color:'#444438',textTransform:'uppercase'}}>Completion</span>
              <span style={{fontSize:10,letterSpacing:'0.06em'}}>
                <span style={{color:'#c8f04a'}}>{done}</span>
                <span style={{color:'#444438'}}>/{actions.length} done · {pct}%</span>
              </span>
            </div>
            <div style={{height:3,background:'#1a1a14',borderRadius:2}}>
              <div style={{height:'100%',width:`${pct}%`,background:'#c8f04a',borderRadius:2,transition:'width 0.6s ease'}}/>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={s.cols}>
        ...
        <section style={s.col}>
          ...
          {actions.length === 0
            ? <p style={s.empty}>No action items</p>
            : <ul style={s.list}>
                {actions.map((a,i) => {
                  ...
                  return (
                    <li key={a.id} className="arow"
                      ...
                      style={{...s.arow, opacity: a.completed ? 0.4 : 1,
                        animationDelay:`${i*0.05}s`}}>
                      <div style={{...s.cb,...(a.completed?s.cbOn:{})}}>
                        {a.completed && <span style={{fontSize:9,color:'#0c0c09',fontWeight:700}}>✓</span>}
                      </div>
                      <div style={{flex:1}}>
                        <p style={{fontSize:13,color:a.completed?'#555548':'#e8e4d0',
                          textDecoration:a.completed?'line-through':'none',
                          lineHeight:1.5,marginBottom:6}}>{a.task}</p>
                        <div style={{display:'flex',gap:10,flexWrap:'wrap',alignItems:'center'}}>
                          {a.owner && a.owner!=='Unassigned' &&
                            <span style={{fontSize:10,color:'#6b7260',letterSpacing:'0.05em'}}>{a.owner}</span>}
                          {dl && <span style={{fontSize:10,color:dl.color,letterSpacing:'0.05em'}}>{dl.label}</span>}
                        </div>
                      </div>
                    </li>
                  )
        ...
        <section style={s.col}>
          ...
          {decisions.length === 0
            ? <p style={s.empty}>No decisions</p>
            : <ul style={s.list}>
                {decisions.map((d,i) => (
                  <li key={i} style={{...s.drow, animationDelay:`${i*0.05}s`}}>
                    <span style={s.dn}>{String(i+1).padStart(2,'0')}</span>
                    <div style={{flex:1}}>
                      <div style={{width:20,height:2,background:'#c8f04a',marginBottom:8}}/>
                      <p style={{fontSize:13,color:'#a8a890',lineHeight:1.65}}>{d}</p>
                    </div>
                  </li>
                ))}
              </ul>
              ...
              <ul style={s.list}>
                {followUps.map((f,i) => (
                  <li key={i} style={{...s.drow, animationDelay:`${i*0.05}s`}}>
                    <span style={s.dn}>{String(i+1).padStart(2,'0')}</span>
                    <div style={{flex:1}}>
                      <div style={{width:20,height:2,background:'#3a3a2e',marginBottom:8}}/>
                      <p style={{fontSize:13,color:'#6b7260',lineHeight:1.65}}>{f}</p>
                    </div>
                  </li>
                ))}
              </ul>
          ...
          {meeting.transcript && (
            <>
              ...
              <pre style={{fontSize:11,color:'#555548',lineHeight:1.8,
                whiteSpace:'pre-wrap',background:'#111108',
                border:'1px solid #1e1e14',borderRadius:6,
                padding:'16px',maxHeight:300,overflowY:'auto'}}>
                {meeting.transcript}
              </pre>
            </>
          )}
...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .arow:hover{background:#1a1a12 !important;border-color:#3a3a2a !important;}
  .back:hover{color:#f0ece0 !important;border-color:#4a4a3a !important;}
`
...
const s = {
  ...
  eyebrow: {fontSize:9,letterSpacing:'0.18em',color:'#444438',textTransform:'uppercase',marginBottom:10},
  title:   {fontFamily:"'Playfair Display',serif",fontSize:'clamp(20px,3vw,36px)',fontWeight:700,
            color:'#f0ece0',letterSpacing:'-0.3px',marginBottom:12,lineHeight:1.15},
  summary: {fontSize:13,color:'#8a8a74',lineHeight:1.75,letterSpacing:'0.02em',maxWidth:540},
  statsRow:{display:'flex',alignItems:'center',background:'#141410',border:'1px solid #2a2a20',
            borderRadius:6,padding:'18px 20px',flexShrink:0},
  ...
  statL:   {fontSize:9,letterSpacing:'0.12em',color:'#555548',textTransform:'uppercase'},
  statDiv: {width:1,height:32,background:'#2a2a20'},
  prog:    {maxWidth:500},

  cols:    {display:'grid',gridTemplateColumns:'1fr 1fr',gap:0,
            minHeight:'calc(100vh - 200px)'},
  col:     {padding:'28px 36px',borderRight:'1px solid #1a1a14',
            animation:'fadeUp 0.35s ease 0.1s both'},
  colHdr:  {display:'flex',justifyContent:'space-between',alignItems:'center',
            marginBottom:18,paddingBottom:14,borderBottom:'1px solid #1a1a14'},
  colHdr2: {display:'flex',justifyContent:'space-between',alignItems:'center',
            marginTop:32,marginBottom:18,paddingBottom:14,borderBottom:'1px solid #1a1a14'},
  ...
  empty:   {fontSize:12,color:'#444438',letterSpacing:'0.05em',padding:'20px 0'},
  list:    {listStyle:'none',display:'flex',flexDirection:'column',gap:6},

  arow:    {display:'flex',alignItems:'flex-start',gap:12,background:'#111108',
            border:'1px solid #1e1e14',borderRadius:6,padding:'13px 14px',cursor:'pointer',
            transition:'background 0.15s,border-color 0.15s',animation:'fadeUp 0.3s ease both'},
  cb:      {width:17,height:17,border:'1.5px solid #3a3a2e',borderRadius:3,flexShrink:0,
            marginTop:2,display:'flex',alignItems:'center',justifyContent:'center',
            transition:'all 0.15s'},
  cbOn:    {background:'#c8f04a',borderColor:'#c8f04a'},

  drow:    {display:'flex',gap:14,alignItems:'flex-start',padding:'14px',
            background:'#111108',border:'1px solid #1e1e14',borderRadius:6,
