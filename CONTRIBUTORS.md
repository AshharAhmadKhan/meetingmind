# Contributors

## Project Creator & Lead Developer

**Ashhar Ahmad Khan**
- **Role:** Creator, Lead Developer, Architect
- **Email:** itzashhar@gmail.com
- **GitHub:** [@AshharAhmadKhan](https://github.com/AshharAhmadKhan)
- **LinkedIn:** [linkedin.com/in/ashhar-ahmad-khan](https://www.linkedin.com/in/ashhar-ahmad-khan/)
- **Contributions:**
  - Complete system architecture and design
  - All 18 Lambda functions implementation
  - Frontend React application
  - AI/ML pipeline with Bedrock integration
  - Infrastructure as Code (AWS SAM)
  - Documentation and testing framework
  - Competition submission and demo preparation

---

## Special Thanks


### AWS Community
- AWS AIdeas Competition organizers and community
- AWS Bedrock team for multi-model AI capabilities
- AWS Serverless team for excellent documentation

---

## How to Contribute

Interested in contributing? Check out [CONTRIBUTING.md](CONTRIBUTING.md) for guidelines.

---

## Recognition

If you find this project helpful, please:
- ⭐ Star the repository on GitHub
- 🔗 Share it with your network
- 💬 Provide feedback via issues
- 🗳️ Vote for us in the AWS AIdeas Competition (March 13-20, 2026)

---

**Note:** This project was built for the AWS AIdeas Competition 2026. All code, architecture, and documentation are original work by Ashhar Ahmad Khan.

**Last Updated:** February 21, 2026
